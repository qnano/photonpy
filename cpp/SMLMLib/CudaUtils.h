...
struct DLL_EXPORT CudaMemoryCounter {
	static int AddPinnedMemory(int amount);
	static int AddDevicePitchedMemory(int amount);
	static int AddDeviceMemory(int amount);
};
...
public:
	...
	void Init(size_t s) {
		...
		if (s!=0) {
			...
			CudaMemoryCounter::AddDeviceMemory(int(sizeof(T)*s));
			n = s;
		}
	}
	void Free() {
		if (d) {
			...
			CudaMemoryCounter::AddDeviceMemory(-int(sizeof(T)*n));
			d=0;
		}
	}
...
public:
	...
	void Free() {
		...
		CudaMemoryCounter::AddPinnedMemory(-int(sizeof(T)*n));
		d=0;n=0;
	}
	void Init(size_t n) {
		if (d) Free();
...
public:
	...
	void Init(int w, int h) {
		if (data) {
			...
			CudaMemoryCounter::AddDevicePitchedMemory(-int(sizeof(T)*width*height));
		}
		width = w;
		height = h;
		if (NumPixels() > 0) {
...
void EmptyKernel(cudaStream_t stream);
