...
public:
	...
	{
		return std::vector<Pt>(driftPerFrame, driftPerFrame + NumFrames());
	}
...
{
	typedef Vector<float, D> Vec;
	const Vec* coords = (const Vec*)coords_;
	const Vec* crlb = (const Vec*)crlb_;
	...
	std::copy(drift.begin(), drift.end(), (Vec*)drift_);
	return its;
}
